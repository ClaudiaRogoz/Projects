...
void send_file(char* filename,int numberpack){
 ...
 // fprintf(f,"%s",filename);
  ...
  while (1){
	...
	send_message(&coada[nowack]);
	printf("am retrimis %i",coada[nowack].type); 
	...
	ackrecv[nowack] = 0;
	  ...
	  if(getcrt(x) >= nowack){
		...
		if(notack==0){
		  ...
		  // daca s-a primit un pachet bine si s-a ajuns din nou la inceputul ferestrei mutam fereastra daca 
		  //am primit toate ack urile atunci 
		  if(nowack == where){
		    i=0;
		  ...
		  printf("pachet primit bine darnu prea");  
		...
		ackrecv[getcrt(x)] = 1;
		// retrimitem tot pana la el 
		for(i=nowack;i<numberpack;i++)
		  send_message(&coada[i]);
		for(i=0;i<getcrt(x);i++)
		  send_message(&coada[i]);
		nowack = getcrt(x);
		if(nowack == numberpack -1)
		  nowack = 0;
